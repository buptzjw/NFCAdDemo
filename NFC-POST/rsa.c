...
{
        char *plain;
        unsigned char *N, *d, *c, *res,temp; /* res = n^e mod N */
...
/* X = X*Y mod N */
{
        ...
        // 计算 A = X'*Y/(BETA^true_size) mod N = X*Y mod N
...
int compare(unsigned char *X, unsigned char *Y, int size) // 比较大小
/* X > Y : 1
   X = Y : 0
   X < Y : -1 */
{
